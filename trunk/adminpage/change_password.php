		...
		<div class="sep">
			<table>
				<tr>
					<td colspan="2">
						<?php 
							$idtaikhoan = $_SESSION['authentication']['idtaikhoan'];
							if(isset($_POST['change'])){
								$oldpass = $_POST['oldpass'];
								$newpass1 = $_POST['newpass1'];
								$newpass2 = $_POST['newpass2'];
								$cnn = DB::StaticConnect();
								$rs = DB::ExecuteQuery("select tentk from taikhoan where idtaikhoan = '$idtaikhoan' and matkhau = '".md5($oldpass)."'",$cnn);
								if(mysql_num_rows($rs) > 0){
									if($newpass1 != "" && $newpass2 != "" && $newpass1 == $newpass2){
										$cnn2 = DB::StaticConnect();
										$rs2 = DB::ExecuteQuery("update taikhoan set matkhau = '".md5($newpass1)."' where idtaikhoan = '$idtaikhoan'", $cnn2);
										if($rs2){
