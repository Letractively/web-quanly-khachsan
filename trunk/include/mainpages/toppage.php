<?php

?>
<a href="index.php"><div id="Logotoppage">
    </div></a>
<div id="Languagetoppage">
    <span><a href="index.php">XuanHongHotel.com.vn</a></span>&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp;
    <span>Ngôn ngữ</span>
    <a href="#"><div id="language-en"></div></a>
    <a href="#"><div id="language-vi"></div></a>
</div>
<div id="Menutoppage">
    <ul id="Menutop-list-ul">
        <li>
            <a href="index.php"><div id="Menutop-list-li-first" class="Menutop-list-li">TRANG CHỦ</div></a>
        </li>
        <li>
            <a href="index.php?act=roomkinds"><div class="Menutop-list-li">LOẠI PHÒNG</div></a>
        </li>
        <li>
            <a href="index.php?act=services"><div class="Menutop-list-li">DỊCH VỤ</div></a>
        </li>
        <li>
            <a href="index.php?act=images"><div class="Menutop-list-li">HÌNH ẢNH</div></a>
        </li>
        <li>
            <a href="index.php?act=news"><div class="Menutop-list-li">TIN TỨC</div></a>
        </li>
        <li>
            <a href="index.php?act=feedback"><div class="Menutop-list-li">PHẢN HỒI</div></a>
        </li>
        <li>
            <a href="index.php?act=contact"><div id="Menutop-list-li-last" class="Menutop-list-li">LIÊN HỆ</div></a>
        </li>
    </ul>
    <div style="clear: both;"></div>
</div>
